<template>
  <div class="container">
    ...
    <table class="table">
      ...
      <tbody>
        <tr class="cursor-pointer tr-item" v-for="(item, index) in filteredData" :key="index" @click="selectTask(item.taskTitle, item.taskDescription, item.date, item.status)">
          <td><input type="radio" name="task" :id="`task-${index}`" @change="handleRadioClick(item)" /></td>
